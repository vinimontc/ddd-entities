...
describe("Order repository test", () => {
  ...
  it("should find one order", async () => {
    ...
    expect(orderFounded.id).toBe("123");
    expect(orderFounded.customerId).toBe(customer.id);
    expect(orderFounded.items.length).toBe(2);
  });
  ...
  it("should find all orders", async () => {
    ...
    expect(ordersFounded.length).toBe(2);
    expect(ordersFounded).toContainEqual(order);
    expect(ordersFounded).toContainEqual(order2)
  });
});
